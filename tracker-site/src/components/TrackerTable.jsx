...
export default function TrackerTable({ group }) {
  ...
  useEffect(() => subscribeStudents(group.id, (list) => {
    setStudents(list)
    setError(null)
  }, (issue) => {
  ...
  useEffect(() => subscribeHomeworkLabels(group.id, setHomeworkLabels, (issue) => console.error('Homework labels error', issue)), [group.id])
  ...
  return <>
    <GroupAchievements summary={achievementData.summary} />
    <section className="students-section" aria-labelledby="students-title">
      <div className="section-heading students-heading">
        <div><p className="eyebrow">Live scoreboard</p><h2 id="students-title">{group.name}</h2></div>
